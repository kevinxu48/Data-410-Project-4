...
Random Forest is a Regression technique discussed in DATA 310, which involves picking a random set of points in the data and building regression trees. Sklearn has a Random Forest Regressor class that we import, and there are three hyperparameters to consider: number of trees in the forest, the maximum depth of the tree, and the minimum number of samples required to split an internal node
